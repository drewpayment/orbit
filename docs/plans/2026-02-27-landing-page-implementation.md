...
**Goal:** Build a premium marketing landing page at `/` for unauthenticated visitors, with login/signup CTAs, replacing the Payload CMS placeholder.

**Architecture:** New `(marketing)` route group in Next.js App Router, outside the existing `(frontend)` AuthGuard. The page is a pure React server component with Tailwind — no Payload CMS dependency, no database queries. The existing `(frontend)/page.tsx` Payload placeholder gets replaced with a redirect to `/dashboard`.

**Tech Stack:** Next.js 15 (App Router), React 19, Tailwind v4, Lucide icons, next/font/google (Instrument Serif + Inter), existing shadcn/ui tokens

---

### Task 1: Remove Payload placeholder and resolve route conflict

The current `(frontend)/page.tsx` claims the `/` route. We need to free it up for the marketing page.

**Files:**
- Modify: `orbit-www/src/app/(frontend)/page.tsx`
- Delete: `orbit-www/src/app/(frontend)/styles.css` (Payload placeholder styles)

**Step 1: Replace the Payload placeholder with a dashboard redirect**

Replace the entire contents of `orbit-www/src/app/(frontend)/page.tsx` with:
...
This way, authenticated users who somehow hit the `(frontend)` root still land on `/dashboard`.

**Step 2: Delete the Payload placeholder stylesheet**

```bash
rm orbit-www/src/app/(frontend)/styles.css
```
...
Expected: Build succeeds with no errors about missing styles.css.

**Step 4: Commit**

```bash
git add orbit-www/src/app/(frontend)/page.tsx
git rm orbit-www/src/app/(frontend)/styles.css
git commit -m "refactor: replace Payload placeholder with dashboard redirect"
```
...
**Files:**
- Create: `orbit-www/src/app/(marketing)/layout.tsx`
...
Expected: Build succeeds. No route conflicts yet (no page.tsx in marketing group).

**Step 3: Commit**

```bash
git add orbit-www/src/app/'(marketing)'/layout.tsx
git commit -m "feat: add (marketing) route group layout with Instrument Serif + Inter"
```

---

### Task 3: Build the landing page — Header and Hero

**Files:**
- Create: `orbit-www/src/app/(marketing)/page.tsx`

**Step 1: Create the landing page with Header and Hero sections**
    ...
    <div className="min-h-screen">
      ...
      <section className="relative flex flex-col items-center px-8 pt-20 pb-16 md:px-[120px] md:pt-20 md:pb-[60px] gap-8">
        ...
        <div className="relative z-10 flex flex-col items-center gap-8">
          {/* Badge */}
          <div className="flex items-center gap-2 rounded-full border border-[#FF5C0033] bg-[#FF5C0018] px-4 py-1.5">
          ...
          {/* Headline */}
          <h1 className="max-w-[900px] text-center font-[family-name:var(--font-instrument-serif)] text-5xl leading-tight tracking-tight md:text-[64px] md:leading-[1.1] md:tracking-[-1px]">
          ...
          {/* Subheadline */}
          <p className="max-w-[700px] text-center text-base leading-relaxed text-[#ADADB0] md:text-[19px] md:leading-[1.6]">
          ...
          <div className="mt-4 flex h-[400px] w-full max-w-[1100px] items-center justify-center rounded-xl border border-white/5 bg-[#141417] shadow-[0_20px_60px_-4px_rgba(255,92,0,0.08)] md:h-[620px]">
...
Expected: Build succeeds. The `/` route is now served by `(marketing)/page.tsx`.
...
- Header shows logo, nav links, "Log In" and "Get Started" buttons
- Hero displays headline in Instrument Serif, badge, CTAs
- Feature grid shows 4 cards with colored icons
- How It Works shows 3 numbered steps
- Final CTA and footer render correctly
- "Log In" and "Get Started" links navigate to `/login`

**Step 4: Commit**

```bash
git add orbit-www/src/app/'(marketing)'/page.tsx
git commit -m "feat: add marketing landing page with hero, features, how-it-works, CTA, footer"
```

---

### Task 4: Add smooth scroll behavior for anchor links

The header nav links use `#features`, `#how-it-works`, `#get-started` anchors. We need smooth scroll.

**Files:**
- Modify: `orbit-www/src/app/(marketing)/layout.tsx`

**Step 1: Add smooth scroll to the html element**

In the `<html>` tag in `orbit-www/src/app/(marketing)/layout.tsx`, add `className="scroll-smooth"`:

```tsx
<html lang="en" className="scroll-smooth" suppressHydrationWarning>
```

**Step 2: Verify smooth scroll works**

```bash
cd orbit-www && pnpm dev
```

Click "Features" in the nav — page should smooth-scroll to the features section.

**Step 3: Commit**

```bash
git add orbit-www/src/app/'(marketing)'/layout.tsx
git commit -m "feat: enable smooth scroll for landing page anchor links"
```

---

### Task 5: Add stagger-reveal animations

Use the existing `stagger-item` CSS utilities from `globals.css` to animate sections on load.

**Files:**
- Modify: `orbit-www/src/app/(marketing)/page.tsx`

**Step 1: Add stagger classes to the hero content**

In the Hero section, add `stagger-reveal` to the container div and `stagger-item` to the badge, headline, subheadline, CTA row, and screenshot:

```tsx
{/* Replace the hero inner div with: */}
<div className="relative z-10 flex flex-col items-center gap-8 stagger-reveal">
  ...
  {/* Headline */}
  <h1 className="stagger-item max-w-[900px] ...">
  {/* Subheadline */}
  <p className="stagger-item max-w-[700px] ...">
  {/* CTAs */}
  <div className="stagger-item flex items-center gap-4">
  {/* Screenshot */}
  <div className="stagger-item mt-4 flex ...">
```
...
Hard-refresh the page. Hero elements should fade in with staggered delays.

**Step 3: Commit**

```bash
git add orbit-www/src/app/'(marketing)'/page.tsx
git commit -m "feat: add stagger-reveal animations to landing page hero"
```

---

### Task 6: Verify end-to-end flow and build
...
**Step 3: Verify the auth flow still works**

Start dev server and verify:
1. Unauthenticated visit to `/` → shows marketing landing page (NOT redirected to `/login`)
2. Click "Log In" → navigates to `/login`
3. Click "Get Started" → navigates to `/login`
4. After login → redirected to `/dashboard`
5. Visit `/dashboard` without auth → redirected to `/login` (AuthGuard still works)

**Step 4: Final commit**

```bash
git add -A
git commit -m "chore: verify landing page build and auth flow"
```
...
| Task | What | Files |
|------|------|-------|
| 1 | Remove Payload placeholder | `(frontend)/page.tsx`, `(frontend)/styles.css` |
| 2 | Create (marketing) layout | `(marketing)/layout.tsx` |
| 3 | Build landing page | `(marketing)/page.tsx` |
| 4 | Smooth scroll | `(marketing)/layout.tsx` |
| 5 | Stagger animations | `(marketing)/page.tsx` |
| 6 | E2E verification | — |
