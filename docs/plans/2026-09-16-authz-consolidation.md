...
- Area PRs: #143 catalog, #144 workspaces, #145 kafka, #146 self-service (all → `chore/authz-phase-c`). Leftover sweep on the integration branch closed the last 18 lint sites and deleted the final two shims; repo-wide restricted warnings = 0 with five documented `eslint-disable` sites for genuine Better-Auth admin API calls (register, setup, platform/users actions, OIDC metadata route, oauth2-proxy seed script).
- Real bugs found by the migration (all masked locally by the dev account's identical ids): Better-Auth ids written into `relationTo: 'users'` fields in ~15 write sites (kafka requests/service accounts/shares/topics, launches, builds, templates, skeletons, action runs, cloud accounts); "mine" filters and creator checks comparing the wrong id (catalog APIs, template lists, launch approvers); infra-agent pages and actions and `/platform/approvals` keying membership on the Payload id (404 / empty for every non-dev account); client-suppliable `userId` overriding session identity (automations, catalog search, join-request, membership-status); roster actions and `/admin/workspaces` with no server-side gate; `/platform/kafka/*` pages gated on login only.
- Adversarial review (2026-09-17, 12 findings): guard default regression in Catalog* delete, ungated `checkMembershipStatus`, residual `userId` gate in catalog types, lost admin bypass in scaffolder approval, dead params, cosmetic short-circuit, stale comments, vacuous tests (`check` mocked always-allow; fixtures with identical ids), and eleven pre-existing ungated server actions in migrated files (apps manifest, deployments status, launch approve/deorbit/abort, kafka topic delete/approve). All fixed on the integration branch except the two items below.
- Accepted, not coded: (a) **no data backfill** for the id fixes: no production data exists and the only dev account has identical ids, so no stored row holds a mismatched id; Phase E's reconciler will include a `relationTo: 'users'` sweep anyway as belt-and-braces. (b) **Platform-admin bypass widening**: every migrated membership check now goes through the policy, which grants platform admins everything. That is the documented model (SOP), but the area PR bodies under-report where it landed; the full list is: kafka actions (applications, application-requests, quotas, service-accounts, topic-shares, topic-catalog, workspace kafka actions, pending-approvals and shared/outgoing pages), azure-devops `requireMember`, all infra-agent gates and pages, legacy templates edit/view pages. Three sites deliberately keep membership-only semantics: `lib/grpc/auth-interceptor.ts` (`wid` claim), kafka `shared/incoming` page, kafka `clusters/[clusterId]` page.
- The gRPC `adm` claim now reads `users.role` (Payload) via the Actor instead of the mirrored Better-Auth `role`; Payload is the role authority (`platform/users` writes it and mirrors to Better-Auth), so this is the intended source.
- Live verification (2026-09-17) on the integration branch with a second dev user whose Payload and Better-Auth ids differ (member.test@example.com, plain member of Engineering): 20 member pages and 12 admin pages render or redirect as intended; an API created by the member stores `createdBy` = Payload id. One regression found and fixed: workspace settings / kafka catalog pages 500'd for owners because the cached membership lookup ran with `overrideAccess: false` against the collection read rule (now `workspaceRole()`; the roster helper with that option was removed).
- Delivered as PR #147 (integration → `chore/authz-phase-c`) stacked under PR #142 (→ main). Merge #147 first (marks #143–#146 merged), then #142.
- Follow-ups noted, not in scope: non-member workspace page lists the workspace's apps and APIs to any signed-in user (pre-existing, workspace read is org-wide by design); Phase D flips the lint rules to error and adds the CI grep test; Phase E's reconciler should also sweep `relationTo: 'users'` fields for Better-Auth ids.
