...
const STEP_VARIANT: Record<StepStatus, 'default' | 'secondary' | 'destructive' | 'outline'> = {
  ...
  // short-circuits it to a "skipped" step + an "unsupported" plan entry), so
  // this state is never actually rendered here — included only so the map
  // stays total against the collection's step-status enum.
