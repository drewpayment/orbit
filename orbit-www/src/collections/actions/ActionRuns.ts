...
export const ActionRuns: CollectionConfig = {
  ...
  fields: [
    ...
    },
    {
      ...
      index: true,
      admin: { description: 'Scaffolder (Phase 1): the definition version this run executed.' },
    },
    {
      name: 'dryRun',
      type: 'checkbox',
      defaultValue: false,
      admin: { description: 'True for a Preview/plan run — no side effects.' },
    },
    {
      name: 'steps',
      type: 'array',
      admin: { readOnly: true, description: 'Per-step live status, written by ScaffolderWorkflow progress updates.' },
      fields: [
    ...
    },
    {
      name: 'plan',
      type: 'json',
      admin: { readOnly: true, description: 'PlannedChange[] from a dry run, or null.' },
    },
    {
      name: 'artifactsPrefix',
      type: 'text',
      admin: { readOnly: true, description: 'MinIO key prefix, e.g. scaffolder-runs/{runId}/.' },
    },
    {
      name: 'workflowId',
      type: 'text',
