// orbit-www/src/lib/scaffolder/visible-if.ts
/**
 * `ui:visibleIf` expression evaluator — server-side copy.
 *
 * **This MUST stay behaviourally identical to
 * `orbit-www/src/components/forms/schema-form/visible-if.ts` (PR #103,
 * `feat/schema-form`).** That module is the authoring UI's live copy
 * (unregisters hidden fields from client-side validation); this one backs
 * `validateRunParameters` in `templates/authoring-actions.ts` so a
 * server-side "required" check doesn't reject a field the UI never showed
 * (and never submitted) because its `ui:visibleIf` evaluated false. The two
 * were written independently — at the time this file was added, the
 * schema-form branch wasn't merged into this worktree — rather than forked
 * from a shared import. Once both land on `main`, promote ONE of them to a
 * shared location the other imports, instead of letting two copies drift.
 * Any semantic change here (or there) must be mirrored in the other AND in
 * both test files.
 *
